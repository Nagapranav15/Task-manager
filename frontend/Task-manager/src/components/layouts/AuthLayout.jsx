...
const AuthLayout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center bg-slate-50 dark:bg-[#070a13] text-slate-900 dark:text-slate-100 font-sans relative overflow-hidden transition-colors duration-300 px-4">
      ...
      <div className="w-full max-w-[500px] flex flex-col justify-between p-8 sm:p-12 z-10 bg-white/40 dark:bg-slate-950/20 backdrop-blur-3xl border border-slate-200 dark:border-slate-900/60 rounded-3xl shadow-xl shadow-slate-200/10 dark:shadow-slate-950/30 transition-all duration-300 my-8">
        ...
        <header className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-white dark:bg-slate-900/60 hover:bg-slate-100 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all cursor-pointer shadow-sm"
        ...
        {/* Content Section (Main Landmark) */}
        <main id="main-content" className="flex flex-col justify-center flex-1">
          {children}
        </main>

        {/* Footer info */}
        <footer className="mt-8 pt-6 border-t border-slate-200/40 dark:border-slate-900/40 text-center">
